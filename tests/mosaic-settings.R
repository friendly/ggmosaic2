...
merged_plot <- ggplot(
  ...
  mosaic_settings(
    expected = "independence", area = "expected", offset = 0.02
  ) +
  mosaic_settings(offset = 0.01) +
  mosaic_settings(expected = NULL)
stopifnot(
  identical(merged_plot$ggmosaic2_settings$offset, 0.01),
  identical(merged_plot$ggmosaic2_settings$area, "expected"),
  "expected" %in% names(merged_plot$ggmosaic2_settings),
  is.null(merged_plot$ggmosaic2_settings$expected)
)

# Clearing a model does not silently reset the independent area setting. The
# resulting expected-area request is invalid until area is also reset.
cleared_model_message <- tryCatch(
  ggplot_build(merged_plot + geom_mosaic()),
  error = conditionMessage
)
stopifnot(grepl(
  "requires a non-NULL `expected`",
  cleared_model_message,
  fixed = TRUE
))
...
stopifnot(
  ...
  inherits(try(mosaic_settings(area = "unknown"), silent = TRUE), "try-error"),
  inherits(try(mosaic_settings(area = c("observed", "expected")), silent = TRUE),
           "try-error"),
