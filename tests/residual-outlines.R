...
stopifnot(
  all(is.na(default_data$colour)),
  all(default_data$linetype == "solid"),
  all(default_data$linewidth == 0.1)
)

shaded_plot <- default_plot + scale_fill_residual()
shaded_data <- ggplot_build(shaded_plot)$data[[1]]

positive <- shaded_data$.residual > 0
negative <- shaded_data$.residual < 0

stopifnot(
  any(positive),
  any(negative),
  all(shaded_data$colour[positive] == "darkblue"),
  all(shaded_data$linetype[positive] == "solid"),
  all(shaded_data$colour[negative] == "darkred"),
  all(shaded_data$linetype[negative] == "dashed"),
  all(shaded_data$linewidth == 0.4)
)
