...
#'
#' Set the divider, gap size, model, and area basis for mosaic layers in a
#' plot. A value set directly in a layer takes priority.
#'
...
#' @param area Values used to construct mosaic rectangles: `"observed"` (the
#'   default) or fitted `"expected"` counts. Expected areas require a
#'   non-`NULL` effective `expected` setting.
#'
#' @details
...
#' layer default (`mosaic()`, `0.01`, `NULL`, or `"observed"`).
#'
#' `divider` and `offset` are inherited by [geom_mosaic()], [stat_mosaic()],
#' [geom_mosaic_text()], [stat_mosaic_text()], [geom_mosaic_jitter()], and
#' [stat_mosaic_jitter()]. `expected` and `area` are inherited by all six
#' constructors. Jitter layers use `expected` only when their effective
#' `area` is `"expected"`.
#'
...
mosaic_settings <- function(divider, offset, expected, area) {
  values <- list()
  ...
  if (!missing(area)) {
    values["area"] <- list(validate_mosaic_setting_area(area))
  }
...
validate_mosaic_setting_area <- function(area) {
  if (!is.character(area) || length(area) != 1L || is.na(area)) {
    stop('`area` must be either "observed" or "expected".', call. = FALSE)
  }
  match.arg(area, c("observed", "expected"))
}
